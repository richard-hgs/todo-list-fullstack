...
import { AppDbLoggerInterface } from "./app-db-logger.interface";
...
export class AppConsoleLogger implements LoggerService, AppDbLoggerInterface {
  ...
  protected updateAndGetTimestampDiff(): string {
    const includeTimestamp =
            AppConsoleLogger.lastTimestampAt && this.options.timestamp;
    const result = includeTimestamp
      ? this.formatTimestampDiff(Date.now() - AppConsoleLogger.lastTimestampAt)
      : '';
    AppConsoleLogger.lastTimestampAt = Date.now();
    return result;
  }
