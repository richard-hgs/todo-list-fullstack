...
import { logout as logoutUser, validateUser } from "@/lib/reducers/authActions";
import {
  createNewTask,
...
function Dashboard() {
  ...
  const actionMessage = useAppSelector(
    (state) => state?.task?.message ?? { type: null, message: null }
  );
  const selectedStatus = useAppSelector(
    (state) => state?.task?.selectedStatus ?? status[0]
  );
  ...
  const toggleTaskCompletion = (index: number) => {
    ...
    dispatch(
      updateTask({
        ...taskAt,
        status: taskAt.status === "Completed" ? "Pending" : "Completed",
      })
    );
  };
  ...
  const setStatus = (status: SelectedStatus) => {
